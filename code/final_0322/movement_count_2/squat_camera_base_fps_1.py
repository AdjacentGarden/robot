...
def background_counting_task(video_source, count_file_path, pid_file_path, tts_mp_q=None):
    ...
    try:
        ...
        while cap.isOpened() and is_running:
            ...
            # 绘制姿势（使用缓存的结果，避免重复检测）
            try:
                if det.last_result and det.last_result.pose_landmarks:
                    mp_drawing.draw_landmarks(frame, det.last_result.pose_landmarks, mp_pose.POSE_CONNECTIONS)
            except Exception as e:
...
class SquatCountingSystem:
    ...
    def start_counting(self, video_source):
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.count_file, self.pid_file, None),  # 改为None
            daemon=True,
        )
        p.start()
        self._process = p
    ...
    def stop_and_summarize(self):
        ...
        print(f"深蹲计数程序结束，您一共做了{final_count}个深蹲")
