...
def process(flag, use_local_model=None):
    ...
    # 设置是否使用本地模型
    use_local_model = use_local_model if use_local_model is not None else USE_LOCAL_MODEL
    
    if use_local_model:
        pose_config['model_asset_path'] = POSE_MODEL_PATH
        print(f"使用本地模型: {POSE_MODEL_PATH}")
    else:
        print("使用在线模型")
