...
class CameraReader:
    ...
    def isOpened(self): return self.running
    def release(self):
        self.running = False
...
def background_gesture_task(camera_id, model_path, result_queue):
...
class GestureRecognitionSystem:
    ...
    def identify_gesture(self, camera_id='/dev/video21'):
        ...
        p = multiprocessing.Process(
            ...
            args=(camera_id, self.model_path, q),
            daemon=True
        )
        p.start()
        result = None
        try:
            result = q.get(timeout=6.0)
        except multiprocessing.queues.Empty:
            print("手势识别进程未能返回结果或已超时")
        p.join()
        print("手势识别进程已退出")
        return result
