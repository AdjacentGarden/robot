...
def background_register_face_task(video_source, db_path, model_path, person_name, tts_mp_q=None):
    import speaker
                    ...
                    if not is_extracting:
                        ...
                        if face_image.size > 0:
                            ...
                            if focus_measure > 10.0:
                                ...
                                cv2.putText(vis, "Blurry", (bx, ma(20, by - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    else:
...
def background_recognize_face_task(video_source, db_path, model_path, tts_mp_q=None):
    import speaker
    speaker.init_mp_queue(tts_mp_q)
    cap = None
    ...
    try:
        ...
        def do_inference(face_img):
            ...
            try:
                ...
                if focus_measure > 10.0 and len(all_known_faces) > 0:
...
class FaceRecognitionSystem:
    ...
    def register_face(self, video_source, person_name):
        import speaker
        print(f"启动人脸注册进程")
        ctx = multiprocessing.get_context('spawn') # 强制使用 spawn
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.db_path, self.model_path, person_name, speaker._mp_q), # 传入队列
            daemon=True
        )
        p.start()
        p.join()
    ...
    def recognize_face(self, video_source):
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.db_path, self.model_path, speaker._mp_q), # 传入队列
            daemon=True
        )
        p.start()
        p.join()
