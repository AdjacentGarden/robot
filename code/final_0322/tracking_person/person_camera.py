...
class MultiBoxTracker:
    MIN_SIZE      = 24.0; BASE_SPEED    = 0.40; STEERING_GAIN = 0.65
...
def background_person_search_task(video_source, detector_path, target_name, tts_mp_q=None):
    ...
    if found: 
        speaker.speak(f"{target_name}在这里")  # 【修改3】使用 speaker.speak

# 【修改4】增加 tts_mp_q 参数，并在头部绑定
def background_person_tracking_task(video_source, detector_path, reid_path, target_name, pid_file_path, tts_mp_q=None):
    ...
    video_path = os.path.join(video_dir, f"track_{session_id}.mp4")
    ...
    try:
        ...
        while cap.isOpened() and is_running:
            ...
            cv2.imshow("OpenBot Person Follower", vis)
            ...
            if tracker.currentState == TrackerState.TRACKING:
                ...
                speaker.speak(f"{target_name}彻底跟丢了")  # 【修改5】使用 speaker.speak
...
class PersonSearchTrackingSystem:
    ...
    def search_person(self, video_source, target_name):
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.detector_model, target_name, speaker._mp_q),
            daemon=True
        )
        p.start()
        p.join() 
    ...
    def start_person_tracking(self, video_source, target_name):
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.detector_model, self.reid_model, target_name, self.pid_file, speaker._mp_q),
            daemon=True,
        )
        p.start()
        self._process = p
