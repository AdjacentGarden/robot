...
class MultiBoxTracker:
    ...
    def force_target(self, rect: RectF, feat: np.ndarray):
        """当人脸确认身份后，强制锁定该人体特征"""
        self.gallery = [feat.copy()]
        self.lastKnown = rect
    ...
    def _is_valid_candidate(self, rect: RectF) -> bool:
        ...
        curr_h = rect.height()
        # ! height restrict 
        if curr_h <= prev_h * 0.85 or curr_h >= prev_h * 1.15:
            return False
        ...
        # ! 
        max_dist = max(self.frameW * 0.3, self.lastKnown.width() * 2)
    ...
    def trackResults(self, results: List[Detection], frame, auto_init=True):
    ...
    def updateTarget(self):
        ...
        if self.currentState == TrackerState.TRACKING and self.lastKnown:
            ...
            is_stationary = abs(self.currentL) < 0.03 and abs(self.currentR) < 0.03
...
# =============================================================================
# 面向“定向寻找行人”的后台任务 (融合人脸识别)
# =============================================================================
def background_person_search_task(video_source, detector_path, face_db, face_model, target_name, tts_mp_q=None):
    import speaker
    import sqlite3
    import pickle
    import torch
    ...
    from face.face_camera import _get_padded_face_image, _extract_feature
    ...
    try:
        conn = sqlite3.connect(face_db, check_same_thread=False)
        cursor = conn.cursor()
        cursor.execute("SELECT vector FROM face_vectors WHERE name=?", (target_name,))
        row = cursor.fetchone()
        if not row:
            speaker.speak(f"数据库中没有找到{target_name}的信息，请先注册面部信息")
            return
        target_face_vector = pickle.loads(row[0])
        conn.close()

        facenet = InceptionResnetV1(pretrained=None)
        facenet.load_state_dict(torch.load(face_model, map_location='cpu'), strict=False)
        facenet.eval()
        ...
        speaker.speak(f"开始寻找{target_name}")

        while cap.isOpened() and (time.time() - start_time) < 15.0:
            ret, frame = cap.read()
            if not ret or frame is None:
                time.sleep(0.01)
                continue

            set_motor(board, speed_right=0.35, speed_left=-0.35)
            h, w = frame.shape[:2]
            ...
            if face_results.detections and dets:
                for detection in face_results.detections:
                    bbox = detection.location_data.relative_bounding_box
                    fx_center = (bbox.xmin + bbox.width / 2) * w
                    fy_center = (bbox.ymin + bbox.height / 2) * h

                    matched_person_det = None
                    min_cost = float('inf')

                    for det in dets:
                        if det.rect.left < fx_center < det.rect.right and det.rect.top < fy_center < det.rect.bottom:
                            body_height = det.rect.height()
                            if fy_center > det.rect.top + body_height * 0.40:
                                continue
                            
                            expected_head_cy = det.rect.top + body_height * 0.15
                            expected_head_cx = det.rect.centerX()
                            dist_cost = math.hypot((fx_center - expected_head_cx) * 1.0, 
                                                   (fy_center - expected_head_cy) * 1.5)
                            
                            if dist_cost < min_cost:
                                min_cost = dist_cost
                                matched_person_det = det
                    
                    if matched_person_det and min_cost < matched_person_det.rect.width() * 0.35:
                        face_img = _get_padded_face_image(frame, bbox)
                        if face_img.size > 0:
                            current_vec = _extract_feature(face_img, facenet)
                            sim = np.dot(current_vec, target_face_vector) / (np.linalg.norm(current_vec) * np.linalg.norm(target_face_vector))
                            
                            if sim > 0.70:
                                found = True
                                set_motor(board, 0.0, 0.0)
                                x1, y1 = int(matched_person_det.rect.left), int(matched_person_det.rect.top)
                                x2, y2 = int(matched_person_det.rect.right), int(matched_person_det.rect.bottom)
                                cv2.rectangle(vis, (x1, y1), (x2, y2), (0, 255, 0), 3)
                                cv2.putText(vis, f"FOUND {target_name}!", (x1, max(20, y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    if found:
        ...
        speaker.speak(f"找了一圈，没有看到{target_name}")


# =============================================================================
# 面向“定向人脸->人体”特征交接的连续追踪任务
# =============================================================================
def background_person_tracking_task(video_source, detector_path, reid_path, face_db, face_model, target_name, pid_file, tts_mp_q):
    import speaker
    import sqlite3
    import pickle
    import torch
    ...
    try:
        ...
        row = cursor.fetchone()
        if not row:
            speaker.speak(f"数据库中没有找到{target_name}的信息，请先在菜单中注册面部")
            print(f"数据库中没有找到 {target_name}")
            return
        target_face_vector = pickle.loads(row[0])
        conn.close()
        ...
        speaker.speak(f"开始寻找并跟踪{target_name}")
        has_locked = False
        ...
        while cap.isOpened() and is_running:
                    ...
                    for detection in face_results.detections:
                        ...
                        if matched_person_det and min_cost < matched_person_det.rect.width() * 0.35:
                            ...
                            if face_img.size > 0:
                                ...
                                if sim > 0.70: 
                                    speaker.speak(f"找到{target_name}了，已接管跟踪")
                                    crop = tracker._crop(frame, matched_person_det.rect)
                                    body_feat = reid.run(crop)
                                    tracker.force_target(matched_person_det.rect, body_feat)
                                    has_locked = True
                                    break 
                
                tracker.trackResults(dets, frame, auto_init=False)
                
            else:
                tracker.trackResults(dets, frame, auto_init=True)
            ...
            if tracker.currentState == TrackerState.IDLE and has_locked:
                speaker.speak(f"{target_name}彻底跟丢了，重新寻找")
        ...
        print("特定人物追踪进程已退出并释放资源")


class PersonSearchTrackingSystem:
    def __init__(self, detector_path=DETECTOR_MODEL, reid_path=REID_MODEL, face_db=None, face_model=None):
        ...
        self.face_db = face_db if face_db else os.path.join(parent_dir, 'test_faces.db')
        self.face_model = face_model if face_model else os.path.join(parent_dir, 'model', '20180402-114759-vggface2.pt')
    ...
    def search_person(self, video_source, target_name):
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.detector_model, self.face_db, self.face_model, target_name, speaker._mp_q),
            daemon=False
        )
        p.start()
        p.join()
    ...
    def start_person_tracking(self, video_source, target_name):
        import speaker
        print(f"启动定向人脸+人体身份交接追踪: {target_name}")
        if self._process is not None and self._process.is_alive():
            print("人物追踪任务已经在后台运行中，请先停止")
            return
        ...
        p = ctx.Process(
            ...
            args=(video_source, self.detector_model, self.reid_model, self.face_db, self.face_model, target_name, self.pid_file, speaker._mp_q),
            daemon=False,
        )
        p.start()
        self._process = p
    ...
    def start_specific_person_tracking(self, video_source, face_db, face_model, target_name):
        self.face_db = face_db
        self.face_model = face_model
        self.start_person_tracking(video_source, target_name)
