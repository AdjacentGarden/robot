...
class RKNNModel(BaseModel):
    ...
    def init_model(self, model_path, target, device_id):
        ...
        runtime_lib = "/usr/lib64/librknnrt.so"
        if not os.path.exists(runtime_lib):
            raise RuntimeError(
                "缺少 RKNN 运行时库 /usr/lib64/librknnrt.so。\n"
                "请在设备上执行：\n"
                "  sudo mkdir -p /usr/lib64\n"
                "  sudo curl -fL -o /usr/lib64/librknnrt.so "
                "https://raw.githubusercontent.com/airockchip/rknn-toolkit2/master/rknpu2/runtime/Linux/librknn_api/aarch64/librknnrt.so\n"
                "  sudo ln -sf /usr/lib64/librknnrt.so /usr/lib/librknnrt.so\n"
                "  sudo ldconfig"
            )
    ...
    def run_encoder(self, x):
        ...
        # 修改这里：显式指定 data_format 并将 inputs 包装成 list
        out = self.encoder.inference(inputs=self.encoder_input, data_format=['nhwc' for _ in self.encoder_input])
...
def run_model(model, audio_data, sample_rate=16000):
...
if __name__ == "__main__":
    ...
    # basic params
    parser.add_argument("--encoder_model_path", type=str, default="/home/test/code/final_0418/llm/model/encoder-epoch-99-avg-1.rknn", help="encoder model path, could be .onnx or .rknn file")
    parser.add_argument("--decoder_model_path", type=str, default="/home/test/code/final_0418/llm/model/decoder-epoch-99-avg-1.rknn", help="decoder model path, could be .onnx or .rknn file")
    parser.add_argument("--joiner_model_path", type=str, default="/home/test/code/final_0418/llm/model/joiner-epoch-99-avg-1.rknn", help="joiner model path, could be .onnx or .rknn file")
    ...
    audio_data, sample_rate = sf.read("/data/zipformer/tts/output_audio.wav")
