...
POSE_MODEL_TYPE = 'lite'  # 可修改为 'full' 或 'heavy'
POSE_MODEL_PATHS = {
...
POSE_MODEL_PATH = POSE_MODEL_PATHS.get(POSE_MODEL_TYPE, POSE_MODEL_PATHS['lite'])
USE_LOCAL_MODEL = os.path.exists(POSE_MODEL_PATH)
...
class SquatDet:
    def __init__(self, pose_samples_folder=POSE_SAMPLES_DIR, use_local_model=USE_LOCAL_MODEL):
        # 配置MediaPipe Pose模型参数
        pose_config = {
        ...
        # 如果使用本地模型且模型文件存在，则设置本地模型路径
        if use_local_model:
            pose_config['model_asset_path'] = POSE_MODEL_PATH
            print(f"使用本地MediaPipe Pose模型: {POSE_MODEL_PATH}")
        else:
            print("使用在线MediaPipe Pose模型")
        
        self.pose_tracker = mp_pose.Pose(**pose_config)
    ...
    try:
        ...
        while cap.isOpened() and is_running:
            ...
            if not pose_classification:
                ...
                cv2.imshow("OpenBot Squat Monitor", frame)
                cv2.waitKey(1)
                time.sleep(0.1)
                continue
            ...
            # 绘制姿势
            try:
                image = squat_preproc(frame)
                result = det.pose_tracker.process(image)
                if result.pose_landmarks:
                    mp_drawing.draw_landmarks(frame, result.pose_landmarks, mp_pose.POSE_CONNECTIONS)
            except Exception as e:
                print(f"[绘制异常] {e}, shape={getattr(frame, 'shape', None)}")
...
class SquatCountingSystem:
    ...
    def query_progress(self):
        ...
        if os.path.exists(self.count_file):
            try:
                ...
                pass
        speaker.speak(f"您目前做了{count}个深蹲了")
    ...
    def stop_and_summarize(self):
        ...
        speaker.speak(f"深蹲计数程序结束，您一共做了{final_count}个深蹲")
