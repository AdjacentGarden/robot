...
POSE_MODEL_PATH = os.path.join(POSE_MODEL_DIR, 'pose_landmark_lite.task')
USE_LOCAL_MODEL = os.path.exists(POSE_MODEL_PATH)

def video_process(video_path, flag, use_local_model=USE_LOCAL_MODEL):
    """
    处理视频文件，提取姿势关键点并生成可视化结果
    """
    ...
    pose_tracker = mp_pose.Pose(**pose_config)
    ...
    # Initialize tracker.
    # 删除重复的pose_tracker初始化，因为前面已经创建了
