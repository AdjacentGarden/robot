...
_FINAL_0418_ROOT = os.path.abspath(os.path.join(_CURRENT_DIR, ".."))
_HOST_ROOT = os.path.abspath(os.path.join(_CURRENT_DIR, "..", "..", ".."))
for _path in (_FINAL_0418_ROOT, _HOST_ROOT):
    if _path not in sys.path:
        sys.path.insert(0, _path)

from llm.speaker import speak
from llm import speaker
...
DETECTOR_MODEL = r"../llm/model/yolov8s_rknn3.rknn"
WEIGHT_MODEL   = r"../llm/model/yolov8s_rknn3.weight"

DET_CONF = 0.30
TRACK_SEARCH_TIMEOUT_SEC = 12.0
TRACK_RECORD_DURATION_SEC = 10.0
TRACK_SEARCH_SPIN_SPEED = 0.2
...
try:
    ...
    raise ValueError("loading wheel control module wrong")
...
class RKNNDetector:
    ...
    def __init__(self, path, conf=0.30, core_mask=4):
        self.conf = conf
    ...
    # ----------------------------------------------------------
    # 对外接口（与原 RKNNDetector.detect 完全一致）
    # ----------------------------------------------------------
