...
class OnVTG(nn.Module):
    def __init__(self, opt):
        ...
        self.conf_head = make_head(opt['conf_head'])
    ...
    def fuse_and_predict(self, fpn, fpn_masks, text, text_masks, text_size=None):
        ...
        fpn_conf_logits, _ = self.conf_head(fpn, fpn_masks)
        fpn_offsets, fpn_masks = self.reg_head(fpn, fpn_masks)
            
        return fpn_logits, fpn_offsets, fpn_conf_logits, fpn_masks, fpn, future_fpn_logits, future_fpn_offsets
    ...
    def forward(self, vid, vid_masks, text, text_masks, text_size=None):
        ...
        fpn_logits, fpn_offsets, fpn_conf_logits, fpn_masks_later, fpn_later, future_fpn_logits, future_fpn_offsets = \
            self.fuse_and_predict(fpn, fpn_masks, text, text_masks, text_size)

        return fpn_logits, fpn_offsets, fpn_conf_logits, fpn_masks_later, future_fpn_logits, future_fpn_offsets
