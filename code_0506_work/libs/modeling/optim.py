...
def make_optimizer(model, opt):
    ...
    blacklist_modules = (LayerNorm, Scale, LayerScale, nn.LayerNorm)
    def in_roots(param_name, roots):
        return any(param_name == root or param_name.startswith(root + '.') for root in roots)
    ...
    vid_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('vid_net',))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('vid_net_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('vid_net',))],
         'weight_decay': 0.0,
         'lr': opt.get('vid_net_lr', opt['lr']),
        },
    ]
    ...
    text_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('text_net',))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('text_net_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('text_net',))],
         'weight_decay': 0.0,
         'lr': opt.get('text_net_lr', opt['lr']),
        },
    ]
    ...
    fusion_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('fusion',))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('fusion_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('fusion',))],
         'weight_decay': 0.0,
         'lr': opt.get('fusion_lr', opt['lr']),
        },
    ]
    ...
    cls_head_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('cls_head', 'future_cls_head'))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('cls_head_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('cls_head', 'future_cls_head'))],
         'weight_decay': 0.0,
         'lr': opt.get('cls_head_lr', opt['lr']),
        },
    ]
    ...
    reg_head_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('reg_head', 'future_reg_head'))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('reg_head_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('reg_head', 'future_reg_head'))],
         'weight_decay': 0.0,
         'lr': opt.get('reg_head_lr', opt['lr']),
        },
    ]
    ...
    conf_head_groups = [
        {'params': [param_dict[pn] \
            for pn in sorted(list(decay)) if in_roots(pn, ('conf_head',))],
         'weight_decay': opt['weight_decay'],
         'lr': opt.get('conf_head_lr', opt['lr']),
        },
        {'params': [param_dict[pn] \
            for pn in sorted(list(no_decay)) if in_roots(pn, ('conf_head',))],
         'weight_decay': 0.0,
         'lr': opt.get('conf_head_lr', opt['lr']),
        },
    ]
    ...
    param_groups = (
        ...
        + cls_head_groups + reg_head_groups + conf_head_groups
    )
