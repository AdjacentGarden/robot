...
class SelfDrivingNode(Node):
    ...
    def init_process(self):
        ...
        if not self.get_parameter('only_line_follow').value:
            ...
            set_servo_position(self.joints_pub, 1, ((1, 500), ))  # 初始姿态
        time.sleep(1)
    ...
    def main(self):
        while self.is_running:
            ...
            if self.start:
                ...
                # 检测到斑马线,开启减速标志
                if 450 < self.crosswalk_distance and not self.start_slow_down:  # 只有离斑马线足够近时才开始减速，通过判断斑马线的y轴像素距离
                    self.count_crosswalk += 1
                    if self.count_crosswalk == 3:  # 多次判断，防止误检测
                        self.count_crosswalk = 0
                        self.start_slow_down = True  # 减速标识
                        self.count_slow_down = time.time()  # 减速开始时间戳
                else:  # 需要连续检测，否则重置
                    self.count_crosswalk = 0
                ...
                if self.start_slow_down:
                    if self.traffic_signs_status is not None:
                        ...
                        if self.traffic_signs_status.class_name == 'red' and area < 1000:  # 如果遇到红灯就停车, 如果太近不停
                            self.mecanum_pub.publish(Twist())
                            self.stop = True
                        elif self.traffic_signs_status.class_name == 'green':  # 遇到绿灯，速度放缓通过
                    ...
                    twist.linear.x = self.normal_speed  # 正常速度
                ...
                if 0 < self.park_x and 180 < self.crosswalk_distance:
                    ...
                    if not self.start_park and 235 < self.crosswalk_distance:  # 离斑马线足够近时就开启停车
                        ...
                        self.stop = True
                        threading.Thread(target=self.park_action).start() #开启停车线程 
                
                # 右转及停车补线策略
                if self.detect_turn_right: #检测到右转标识且离斑马线足够近，就开启右转策略
                ...
                if self.turn_right:
                    ...
                    if 0 < y < 400 :
                        ...
                        min_x = cv2.minMaxLoc(binary_image)[-1][0]  #获取最左边的点坐标
                        cv2.line(binary_image, (min_x, y - 20), (w, y - 20), (255, 255, 255), 50)  # 画虚拟线来驱使转弯, 可以通过调整y的大小来控制转弯的时机
                    ...
                    if not self.detect_far_lane:
                        ...
                        if 50 < center < 90:  # 当将要看不到车道线时切换到识别较远车道线
                ...
                result_image, lane_angle, lane_x, max_area = self.lane_detect(binary_image, image.copy())  # 在处理后的图上提取车道线中心
                    ...
                    if self.turn_right:
                        if max_area > 5000:
                            if self.count_turn == 1:
                                self.lane_detect.set_roi(((420, 450, 0, 320, 0.2), (360, 390, 0, 320, 0.3), (300, 330, 0, 320, 0.5)))
                            self.count_turn += 1
                            if self.count_turn > 1 and not self.start_turn:  # 稳定转弯
                                self.start_turn = True
                                self.turn_right = False
                                self.count_turn = 0
                                self.start_turn_time_stamp = time.time()
                            twist.angular.z = twist.linear.x*math.tan(-0.628)/0.213  # 转弯速度
                        else:  # 直道由pid计算转弯修正
                            self.count_turn = 0
