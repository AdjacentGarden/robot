...
class FrontierExplorer(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('replan_interval', 3.0)       # (s) re-evaluate while navigating
        ...
        self.declare_parameter('min_explored_cells', 300)    # don't finish if map too small
        ...
        # Initial spin
        self.declare_parameter('allow_initial_spin', False)  # Ackermann robots should keep this false
    ...
    def _s_waiting(self):
        ...
        next_state = State.INITIAL_SPIN if self.allow_initial_spin else State.EXPLORING
        self.get_logger().info(
    ...
    # ─── INITIAL SPIN ───
    def _s_spin(self):
        if not self.allow_initial_spin:
            self.state = State.EXPLORING
            return
        if time.time() - self.spin_start_time < self.initial_spin_duration:
            cmd = Twist()
    ...
    def _s_exploring(self):
        ...
        best = self._pick(frontiers, rx, ry)
        if not best:
            self.no_frontier_count += 1
            self.get_logger().warn(
    ...
    # ═══════════════════ Frontier Selection ═══════════════════
    def _pick(self, frontiers, rx, ry):
        best, bs = None, -1e9
        for f in frontiers:
            ...
            if d < self.min_frontier_dist:
                continue
            f['dist'] = d
            s = self.gain_weight * math.log1p(f['size']) - self.cost_weight * d
            f['score'] = s
            if s > bs:
                bs = s
                best = f
