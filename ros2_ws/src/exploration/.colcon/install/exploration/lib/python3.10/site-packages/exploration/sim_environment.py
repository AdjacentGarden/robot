...
Generates a random ~100m² indoor map (10m × 10m), simulates:
  - Robot pose integration from /cmd_vel
  - TF broadcast: map -> odom -> base_footprint
  - LaserScan via raycasting on ground-truth map
  - SLAM-like incremental map reveal (only scanned areas are published)
...
class SimEnvironment(Node):
    ...
    def __init__(self):
        ...
        self.map_w_m = self.get_parameter('map_width').value
        self.map_h_m = self.get_parameter('map_height').value
