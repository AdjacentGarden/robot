...
class MapGoalBridge(Node):
    ...
    def __init__(self):
        ...
        self.nav_client = ActionClient(self, NavigateToPose, nav_action_name, callback_group=cb_group)
        self.status_pub = self.create_publisher(String, '/navigation/simple_nav_status', 10)
        
        self.goal_sub = self.create_subscription(
            PoseStamped, goal_topic, self._on_goal, 10, callback_group=cb_group)
        self.legacy_goal_sub = self.create_subscription(
            PoseStamped, legacy_goal_topic, self._on_goal, 10, callback_group=cb_group)
        self.timer = self.create_timer(0.5, self._tick, callback_group=cb_group)
    ...
    def _on_goal(self, msg):
        self.get_logger().info(f'Received goal! frame_id={msg.header.frame_id}, x={msg.pose.position.x:.2f}, y={msg.pose.position.y:.2f}')
